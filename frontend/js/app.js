...
createApp({
  data() {
    return {
      ...
      gptSortOrder: 'asc',
      gptReputationChart: null,
      gptIpReputationChart: null,
      gptAuthChart: null,
      gptSpamChart: null,
      showIPModal: false,
  ...
  computed: {
    ...
    // Sorted GPT table for overview
    sortedGPTTable() {
      if (!this.gptOverviewTable || this.gptOverviewTable.length === 0) {
        return [];
      }

      const sorted = [...this.gptOverviewTable];
      const column = this.gptSortColumn;
      const order = this.gptSortOrder;

      sorted.sort((a, b) => {
        let aVal = a[column];
        let bVal = b[column];

        // Handle reputation values (convert to numeric for sorting)
        if (column === 'domain_reputation') {
          const repMap = { 'HIGH': 4, 'MEDIUM': 3, 'LOW': 2, 'BAD': 1, 'N/A': 0 };
          aVal = repMap[aVal] || 0;
          bVal = repMap[bVal] || 0;
        } else if (column === 'ip_reputation') {
          // IP reputation is now an object with breakdown
          const repMap = { 'HIGH': 4, 'MEDIUM': 3, 'LOW': 2, 'BAD': 1 };

          // For objects, take the highest reputation value
          const getHighestRep = (ipRep) => {
            if (!ipRep || typeof ipRep !== 'object' || Object.keys(ipRep).length === 0) {
              return 0;
            }
            return Math.max(...Object.keys(ipRep).map(rep => repMap[rep] || 0));
          };

          aVal = getHighestRep(aVal);
          bVal = getHighestRep(bVal);
        }
  ...
  methods: {
    ...
    // ============================================================================
    // GOOGLE POSTMASTER TOOLS (GPT) METHODS
    // ============================================================================
    ...
    onDomainChange() {
      // Triggered when domain input changes
      // If empty or "All Domains", load overview
      if (!this.gptSelectedDomain || this.gptSelectedDomain === '' || this.gptSelectedDomain === 'All Domains (Overview)') {
        this.gptSelectedDomain = '';
        this.loadGPTData();
      } else {
        // Load details for selected domain
        this.loadGPTData();
      }
    },
    ...
    getGPTChangeColor(domain) {
      ...
      if (change.change_type === 'degraded') return 'var(--red-500)';
      if (change.change_type === 'improved') return 'var(--green-500)';
      if (change.change_type === 'ip_changed') return 'var(--blue-500)';
      if (change.change_type === 'spam_increase') return 'var(--orange-500)';
      return 'var(--blue-500)';
    },
    ...
    destroyGPTCharts() {
      ...
      if (this.gptReputationChart) {
        this.gptReputationChart.destroy();
        this.gptReputationChart = null;
      }
      if (this.gptIpReputationChart) {
        this.gptIpReputationChart.destroy();
        this.gptIpReputationChart = null;
      }
      if (this.gptAuthChart) {
        this.gptAuthChart.destroy();
        this.gptAuthChart = null;
      }
      if (this.gptSpamChart) {
        this.gptSpamChart.destroy();
        this.gptSpamChart = null;
      }
    },

    createGPTCharts() {
      if (!this.gptDomainDetails || !this.gptDomainDetails.trends) return;

      const trends = this.gptDomainDetails.trends;
      if (!trends.dates || trends.dates.length === 0) return;
      ...
      this.$nextTick(() => {
        ...
        if (repCtx) {
          this.gptReputationChart = new Chart(repCtx, {
            type: 'line',
            data: {
              labels: trends.dates,
              datasets: [{
                label: 'Domain Reputation',
                data: trends.domain_reputation,
            ...
            options: {
              ...
              plugins: {
                ...
                tooltip: {
                  callbacks: {
                    label: (context) => {
                      const repMap = { 4: 'HIGH', 3: 'MEDIUM', 2: 'LOW', 1: 'BAD', 0: 'UNKNOWN' };
                      return `${context.dataset.label}: ${repMap[context.parsed.y] || context.parsed.y}`;
                    }
                  }
                }
              },
        ...
        // IP Reputation Chart - Stacked Bar Chart with Breakdown
        const ipRepCtx = document.getElementById('gptIpReputationChart');
        if (ipRepCtx && trends.ip_reputation_breakdown) {
          const breakdown = trends.ip_reputation_breakdown;

          this.gptIpReputationChart = new Chart(ipRepCtx, {
            type: 'bar',
            data: {
              labels: trends.dates,
              datasets: [
            ...
            options: {
              ...
              plugins: {
                ...
                tooltip: {
                  callbacks: {
                    label: (context) => {
                      return `${context.dataset.label}: ${context.parsed.y} IP(s)`;
                    }
                  }
                }
              },
              scales: {
                ...
                y: {
                  ...
                  title: {
                    display: true,
                    text: 'Number of IPs in each Reputation Group'
                  },
                  ticks: {
                    callback: (value) => {
                      return value;
        ...
        if (authCtx) {
          this.gptAuthChart = new Chart(authCtx, {
            type: 'line',
            data: {
              labels: trends.dates,
              datasets: [
        ...
        if (spamCtx) {
          this.gptSpamChart = new Chart(spamCtx, {
            type: 'line',
            data: {
              labels: trends.dates,
              datasets: [{
  ...
  watch: {
    ...
    gptTimePeriod() {
      // When time period changes, reload data if we have a domain selected or overview loaded
      if (this.activeView === 'gpt' && this.gptAuthorized) {
        if (this.gptSelectedDomain) {
          // Reload domain details with new time period
          this.loadGPTData();
        }
      }
    }
